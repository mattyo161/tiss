...
# @version refs, spec parsing, and self tree install.
...
assertMatch "self tree list shows the package" 'devops' "$("$TISS_BIN" self tree list 2>/dev/null)"
...
assertMatch "disabled package surfaced in list" 'installed, disabled' "$("$TISS_BIN" self tree list 2>/dev/null)"
...
# --- self tree install is the long-form spelling ----------------------------------------
"$TISS_BIN" -devops >/dev/null 2>&1
assertMatch "self tree install enables too" 'devops' "$("$TISS_BIN" self tree install devops 2>/dev/null)"
...
r="$("$TISS_BIN" self tree resolve devops 2>/dev/null)"
...
r="$("$TISS_BIN" self tree resolve newpkg@v9 2>/dev/null)"
...
"$TISS_BIN" self tree add prod --repo "$bare2" --branch env/prod >/dev/null 2>&1
assertEq "custom-mapped tree routes" "prod v0.9.0" "$("$TISS_BIN" prodhello 2>/dev/null)"
r="$("$TISS_BIN" self tree resolve prod 2>/dev/null)"
...
lj="$("$TISS_BIN" self tree list --json 2>/dev/null)"
