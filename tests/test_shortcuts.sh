...
# --- add: definition + shim --------------------------------------------------
"$TISS_BIN" shortcuts add sd saveData >/dev/null 2>&1
...
"$TISS_BIN" shortcuts add rd readData >/dev/null 2>&1
...
"$TISS_BIN" shortcuts add cfgpath config path >/dev/null 2>&1
...
"$TISS_BIN" shortcuts add cfgl config list >/dev/null 2>&1
...
warn="$(TISS_LOG_LEVEL=WARN "$TISS_BIN" shortcuts add ls lsData 2>&1 >/dev/null)"
assertMatch "collision with a real tool warns" 'shadows' "$warn"
"$TISS_BIN" shortcuts remove ls >/dev/null 2>&1
assertExit "name 'tiss' refused" 2 "$TISS_BIN" shortcuts add tiss tf plan
assertExit "invalid characters refused" 2 "$TISS_BIN" shortcuts add 'a/b' tf plan
assertExit "leading dash refused" 2 "$TISS_BIN" shortcuts add -x tf plan

# --- list --------------------------------------------------------------------------
lst="$("$TISS_BIN" shortcuts list 2>/dev/null)"
assertMatch "list shows name, expansion, status, source" 'sd +tiss saveData +ok +user' "$lst"

# --- remove ------------------------------------------------------------------------
"$TISS_BIN" shortcuts remove rd >/dev/null 2>&1
assertFileMissing "removed shortcut's shim pruned" "$TISS_SHIMS/rd"
assertExit "removing an unknown shortcut fails" 2 "$TISS_BIN" shortcuts remove nope

# --- sync reconciles hand-edits, never touches foreign files ------------------------
echo 'zz = lsData' >>"$TISS_CONFIG/shortcuts"
"$TISS_BIN" shortcuts sync >/dev/null 2>&1
...
echo 'acmepath = config path' >"$tree/etc/shortcuts"
TISS_PATH="$tree" "$TISS_BIN" shortcuts sync >/dev/null 2>&1
...
assertExit "removing a tree-owned shortcut errors" 2 \
  env TISS_PATH="$tree" "$TISS_BIN" shortcuts remove acmepath
echo 'acmepath = config list' >>"$TISS_CONFIG/shortcuts"
assertMatch "user definition beats the tree's" 'SETTING' "$(TISS_PATH="$tree" "$TISS_SHIMS/acmepath" 2>/dev/null)"
"$TISS_BIN" shortcuts remove acmepath >/dev/null 2>&1

# --- edit/path seed from the commented template ----------------------------------------
mv "$TISS_CONFIG/shortcuts" "$TISS_CONFIG/shortcuts.saved"
"$TISS_BIN" shortcuts sync >/dev/null 2>&1 # prune shims of the moved-away file
assertEq "path seeds and prints the file" "$TISS_CONFIG/shortcuts" "$("$TISS_BIN" shortcuts path 2>/dev/null)"
...
# --- init emits the guarded PATH line ---------------------------------------------
emit="$("$TISS_BIN" init 2>/dev/null)"
assertMatch "init emits the shim dir" "$TISS_SHIMS" "$emit"
assertMatch "PATH line is guarded" 'case ":\$PATH:"' "$emit"
onpath="$(bash -c "eval \"\$('$TISS_BIN' init 2>/dev/null)\"
...
assertMatch "doctor flags missing PATH line" 'shim dir not on PATH' \
  "$(TISS_LOG_LEVEL=INFO "$TISS_BIN" doctor 2>&1 || true)"
assertMatch "doctor happy when shims healthy and on PATH" 'ok: shim dir on PATH' \
  "$(TISS_LOG_LEVEL=INFO PATH="$TISS_SHIMS:$PATH" "$TISS_BIN" doctor 2>&1 || true)"
